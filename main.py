...
class MotionPlanner:
    ...
    def generateData(self):
        ...
        self.x_domain = data.generateGridData(self.N_domain, self.limits).to(self.device)
        if self.config["Barrier"]:
            ...
            self.x_init = data.generateGridData(self.N_init, init_range).to(self.device)
            ...
            if self.config["unsafe"]["shape"] == "Circle":
                ...
                self.x_unsafe = data.generateGridData(self.N_unsafe, unsafe_range).to(self.device)
            else:
    ...
    def trainBarrierCertificate(self):
        ...
        start = timeit.default_timer()
        for i in range(max_iters):
            dataset = data.MultiVariableDataset(self.x_init, self.x_domain, self.x_unsafe)
            lyapunov_risk = Loss_Functions.loss_function_v(self.model_v, self.x_domain, torch.tensor(self.X_train,dtype=torch.float32).to(self.device), torch.tensor(self.y_train,dtype=torch.float32).to(self.device), i, self.config)
