...
class MotionPlanner:
    ...
    def trainInitialDynamics(self):
        ...
        self.optimizer_f_state_dict = self.optimizer_f.state_dict()
        # print("MSE of Initial Estimate of Dynamical System: %.4f" % best_mse)
...
if __name__ == "__main__":
    ...
    print_info(f"FOR THE SHAPE : {final_name}")
    ...
    # print("OBTAINING DEMO DATA")
    mp.generate_demo_data()
    # print("DYNAMICAL SYSTEM TRAINING")
    ...
    iters = 1
    # print("CERTIFICATE TRAINING")
    mp.update_config()
    mp.trainCertificate()
    trial = 1
    max_trials = 250
    while trial < max_trials:
        # print("ADDING COUNTEREXAMPLES")
        ...
        if mp.counterexamples_added:
            ...
            # print("SAMPLING-BASED VERIFICATION COMPLETE")
            ...
            if mp.flag_verified:
                ...
                # print("Motion Planning Completed Successfully")
                print_success(f"Total time taken for certificate training and verification: {elapsed_time:.2f} seconds")
                print_info(f"Number of trials for certificate training: {trial}")
                mp.save_all_models()
                mp.final_model_eval()
                # print(f"MSE for test data after certificate training: {mp.mse}")
               ...
               print("CONFORMAL PREDICTION FAILED; RETRAINING CERTIFICATE")
        trial += 1
    if trial == max_trials:
