...
class MotionPlanner:
    ...
    def generate_domain_data(self):
        ...
        while len(self.unsafe_domain) < 2 or len(self.init_domain) < 2:
            ...
            self.init_min = (np.min([self.demos[i].pos[:,0] for i in range(self.total_demos)], axis=0) - self.config["init"]["radius"]).reshape(1,2)
            self.init_min = np.where(self.init_min < -1, -1, self.init_min)
            self.init_max = (np.max([self.demos[i].pos[:,0] for i in range(self.total_demos)], axis=0) + self.config["init"]["radius"]).reshape(1,2)
